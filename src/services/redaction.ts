const SENSITIVE_KEY_PATTERN = /^(access[_-]?token|refresh[_-]?token|id[_-]?token|client[_-]?secret|authorization|password|api[_-]?key|email|phoneNumber|deviceId|podSerial|macAddress|shippingAddress|stripeCustomerId|lastFour|lastFourDigits)$/i;
